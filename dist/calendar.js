    ...
    value: function _attachClickEvent(el) {
      ...
      el.addEventListener('click', function (event) {
        ...
        if (_this4.mode.current === VIEW_MODE) {
          _this4.mergedEvents.forEach(function (e) {
            if (e.id == event.target.dataset.eventId) {
              var baseEvents = e.mergedEvents || [e]; // Retrieve original events
              _this4.options.onEventClicked(baseEvents); // Pass the array of merged events
            }
          });
        }
      });
...
},{"./cellMatrix":2,"./dateManager":3,"./eventDispatcher":4,"./lockedEventDispatcher":5,"./uiManager":6}],2:[function(require,module,exports){
"use strict";
...
var CellMatrix = /*#__PURE__*/function () {
  ...
  return _createClass(CellMatrix, [{
    key: "loadEvents",
...
function _toPropertyKey(t) { var i = _toPrimitive(t, "string"); return "symbol" == _typeof(i) ? i : i + ""; }
function _toPrimitive(t, r) { if ("object" != _typeof(t) || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != _typeof(i)) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }
var DateManager = /*#__PURE__*/function () {
  ...
  return _createClass(DateManager, [{
    key: "generateDays",
...
var EventDispatcher = /*#__PURE__*/function () {
  ...
  return _createClass(EventDispatcher, [{
    ...
    value: function loadEvents(events) {
      ...
      this.mergedEvents.forEach(function (event) {
        _this.addEvent(event);
      });
    }
  }, {
    key: "mergeOverlappingEvents",
    value: function mergeOverlappingEvents() {
      var _this2 = this;
      ...
      Object.keys(eventsByColumn).forEach(function (column) {
        var _this2$mergedEvents;
        var columnEvents = eventsByColumn[column];
        columnEvents.sort(function (a, b) {
        ...
        columnEvents.forEach(function (event) {
          if (merged.length === 0) {
            ...
            var lastMergedEvent = merged[merged.length - 1];
            var lastEndTime = new Date(lastMergedEvent.date.getTime() + lastMergedEvent.duration * 60000);
            var currentStartTime = event.date;
            if (currentStartTime < lastEndTime) {
              // Overlap detected - merge events
              var earliestStart = lastMergedEvent.date < event.date ? lastMergedEvent.date : event.date;
              var latestEnd = new Date(Math.max(lastMergedEvent.date.getTime() + lastMergedEvent.duration * 60000, event.date.getTime() + event.duration * 60000));
        ...
        (_this2$mergedEvents = _this2.mergedEvents).push.apply(_this2$mergedEvents, merged);
      });
    }
  }, {
    key: "isOverlapping",
    value: function isOverlapping(event1, event2) {
      var event1End = event1.date.getTime() + event1.duration * 60000;
      var event2Start = event2.date.getTime();
